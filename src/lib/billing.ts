...
export function getPlanInfo(plan: string) {
  // "trial" is a legacy DB value — display as monthly
  const key = plan === 'trial' ? 'monthly' : plan
  return BILLING_PLANS[key as BillingPlan] ?? { label: plan, color: 'bg-slate-100 text-slate-500', order: -1 }
}
