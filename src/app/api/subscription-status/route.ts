  ...
  if (!error) {
    ...
    if (!check.allowed && userId && userEmail && role) {
      logAuditAction({
        ...
        description: check.orgSuspended
          ? 'Aksesi i bllokuar: organizata është pezulluar nga platforma'
          : 'Aksesi i bllokuar: abonimi ka skaduar ose është anuluar',
      }).catch(() => {})
    }
    ...
    return NextResponse.json({
      ...
      orgSuspended: check.orgSuspended ?? false,
    })
  }
